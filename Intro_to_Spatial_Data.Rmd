...
```{r}
easypackages::libraries("tidyverse", "spatstat", "GISTools", "rgdal", "ggpolypath", "sp", "sf", "tmap", "maptools", "spdep", "tmaptools","spatialreg", "rgeos", "RColorBrewer", "adehabitatHR", "raster", "leaflet", "spatialEco", "tm")
...
MQ1_2010_sp <- spTransform(join, CRS("+proj=utm +zone=18 + south + datum=WGS84"))
...
trees_2012_sp <- spTransform(trees_2012, CRS("+proj=utm +zone=18 + south + datum=WGS84"))
...
TBS_coordinates_sp <- spTransform(TBS_coordinates, CRS("+proj=utm +zone=18 + south + datum=WGS84"))
TBS_coordinates_sf <- st_as_sf(TBS_coordinates_sp, CRS("+proj=utm +zone=18 + south + datum=WGS84"))

#Load and project shapefile for Ecuador
yasuni <- st_read("ec_provinces.shx")
yasuni_sp <- as(yasuni, "Spatial")
proj4string(yasuni_sp) <- CRS("+proj=utm +zone=18 + south + datum=WGS84 +units=m")
yasuni_sf <- st_as_sf(yasuni_sp, CRS("+proj=utm +zone=18 + south + datum=WGS84 +units=m"))

#alternatively, can use the readOGR function to read in Shapefiles
yasuni <- readOGR(dsn = "ec_provinces.shx", layer = "ec_provinces")
yasuni_sp <- as(yasuni, "SpatialPolygonsDataFrame")
proj4string(yasuni_sp) <- CRS("+proj=utm +zone=18 + south + datum=WGS84 +units=m")

#plot TBS point over Ecuador Shapefile
#plot function using sp
plot(yasuni_sp, main = "Tiputini Biodiversity Station, Ecuador")
plot(TBS_coordinates_sp, pch = 19, col = "red", add = TRUE)

#ggplot function using sf
ggplot() + geom_sf(data = yasuni_sf, fill = "wheat") + geom_sf(data = TBS_coordinates_sf, color = "red") + xlab("Longitude") + ylab("Latitude") + ggtitle("Tiputini Biodiversity Station, Ecuador") + coord_sf(crs = 32718)
...
plot(hr_kernel95, main = "Home Range, kernelUD method")
...
```{r}
#Plot group size/focal point (need to replace with Shapefile for TBS)
colors_gs <- c("#f7fbff", "#c6dbef", "#6baed6", "#2171b5", "#08306b")
breaks_gs <- c(0, 5, 10, 15, 20, 25)
countries_layer <- list("sp.polygons", yasuni_sp, fill="wheat")
spplot(MQ1_2010_sp, "group_size", sp.layout = countries_layer, cuts=breaks_gs, col.regions=colors_gs, cex=1, main=list(label="Group Size at each Focal Point \n n=8584", cex=1), key.space = "right", xlim=c(-168998,478794), ylim=c(9444903,10161873))
...
points_agg <- spTransform(points_agg, CRS("+proj=utm +zone=18 + south + datum=WGS84"))
...
tm_shape(polygon) + tm_fill(col = "brown", alpha = .1) + tm_borders(col = "brown", lwd = 2) +
tm_shape(points_agg) +
